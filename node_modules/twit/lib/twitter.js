...
Twitter.prototype.stream = function (path, params) {
  ...
  self.streamingConnection = new StreamingAPIConnection()
  self._buildReqOpts('POST', path, params, true, function (err, reqOpts) {
    if (err) {
      ...
      // surface this on the this.streamingConnection instance (where a user may register their error handler)
      self.streamingConnection.emit('error', err)
      return
    }
    // set the properties required to start the connection
    self.streamingConnection.reqOpts = reqOpts
    self.streamingConnection.twitOptions = twitOptions
  ...
  return self.streamingConnection
}

Twitter.prototype.closeStream = function () {
  var self = this;
